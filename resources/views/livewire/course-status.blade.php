<div class="mt-8">
    <div class="container grid grid-cols-1 lg:grid-cols-3 lg:gap-8">
        <div class="col-span-2">
            ...
            <div class="flex items-center mt-4 cursor-pointer" wire:click="completed">
                ...
                @endif
                <p class="text-sm ml-2">Marcar esta unidad como ulminada</p>
            </div>

            <div class="card mt-2 mb-4">
                <div class="card-body flex text-gray-500 font-bold">
                    {{--  Solo si hay algo almacenado en la propiedad previous se muestra el anterior  --}}
                    @if ($this->previous)
                                        ...
                                        <div>
                                            @if ($lesson->completed)
                                                @if ($current->id == $lesson->id)
                                                    <span class="inline-block w-4 h-4 rounded-full border-2 border-yellow-300 mr-2 mt-1"></span>
                                                @else
                                                    <span class="inline-block w-4 h-4 rounded-full bg-yellow-400 mr-2 mt-1"></span>
                                                @endif
                                            @else
                                                @if ($current->id == $lesson->id)
                                                    <span class="inline-block w-4 h-4 rounded-full border-2 border-gray-400 mr-2 mt-1"></span>
                                                @endif
                                                    <span class="inline-block w-4 h-4 rounded-full bg-gray-400 mr-2 mt-1"></span>
                                            @endif
                                        </div>
